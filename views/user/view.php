...
<div style="text-align: center; background-color: white; line-height: 50px;">
    <ul class="user-act-opt-cont">
        <li class="user-act-opt <?= !in_array(CW::$app->request->get('type'), Update::getAllActivityTypesAsString()) ? 'user-act-selected' : '' ?>"><a class="user-act-link" href="/profile/<?= $model['id'] ?>">All</a></li>
        <li class="user-act-opt <?= CW::$app->request->get('type') === Update::ACTIVITY_TYPE_POST_STR ? 'user-act-selected' : '' ?>"><a class="user-act-link" href="/profile/<?= $model['id'] ?>/posted">Posted</a></li>
        <li class="user-act-opt <?= CW::$app->request->get('type') === Update::ACTIVITY_TYPE_UPVOTE_STR ? 'user-act-selected' : '' ?>"><a class="user-act-link" href="/profile/<?= $model['id'] ?>/upvoted">Upvoted</a></li>
        <li class="user-act-opt <?= CW::$app->request->get('type') === Update::ACTIVITY_TYPE_COMMENT_STR ? 'user-act-selected' : '' ?>"><a class="user-act-link" href="/profile/<?= $model['id'] ?>/commented">Commented</a></li>
    </ul>
</div>
